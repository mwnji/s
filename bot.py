...
math 
bot=Bot("prjjtwkxibcibbgpiuiqliqxtthuyrwk")
list_message_seened = []
    ...
    try:
        ...
        if chats_list != []:
            for chat in chats_list:
                access = chat['access']
                admins = open('gueid.txt','r').read().split('\n')
                ...
                if not m_id in list_message_seened and chat['object_guid'] in admins:
                    ...
                    if text == "/start" :
                        bot.sendMessage(chat['object_guid'],'id chanel mored nazar ro ba {@} vared konid mannd >> @monji024')
                    elif text.startswith("@"):
                        id = text.split("@")[1]
                        guid_of_channel = bot.getInfoByUsername(id)['data']['channel']['channel_guid']
                        bot.sendMessage(chat['object_guid'],'capshen ro vared konid mesal >> cap (کپشن)')
                    elif text.startswith("cap ("):
                        caption = id = text.split("(")[1][:-1]
                        bot.sendMessage(chat['object_guid'],'format fail ro vare konid mannd >> -name.mp4')
                    elif text.startswith("-"):
                        new_name = f" {text}"
                        bot.sendMessage(chat['object_guid'],'post mored nazar ro vared konid♻️')
                        ...
                        bot.sendMessage(chat['object_guid'],'uplode fail shro shod >>0')
                                ...
                                while True:
                                    try:
                                        ...
                                        if a > tweny_five and is_tweny_five == False:
                                            bot.sendMessage(chat['object_guid'],'fil 25% uplode shde♻️')
                                            is_tweny_five = True
                                        elif a > fifty and is_fifty == False:
                                            bot.sendMessage(chat['object_guid'],'fil 50% uplode shode♻️')
                                            is_fifty = True
                                        elif a > seventy_five and is_seventy_five == False:
                                            bot.sendMessage(chat['object_guid'],'fil 75% uplode shode♻️')
                        ...
                        if j != None and type(j) == str:
                            ...
                            bot.sendMessage(chat['object_guid'], 'fil dar ' +mir+ ' uplod shod')
                            print('send file'+mir)
                        else:
